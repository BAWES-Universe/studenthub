...
class m200813_122731_transfer_total extends Migration
{
    ...
    {
        $transferFiles = Yii::$app->db->createCommand('SELECT * from transfer_file where transfer_amount = 0 OR 
            transfer_amount IS NULL')->queryAll();
        ...
        foreach($transferFiles as $transferFile) {
            ...
            $transfer_amount = \common\models\TransferCandidate::find()
               ...
               ->andWhere(['transfer_file_id' => $transferFile['transfer_file_id']])
               ->scalar();
